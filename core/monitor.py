...
# watchlist 归一化 + 停牌过滤（2026-07-24：suspended_until/status 字段，扫描前过滤停牌标的）
...
# 归一化标的结构（含停牌状态）：{code: {name, status, suspended_until}}
# monitor 扫描前用 active_symbols(WATCH_ENTRIES) 过滤停牌标的；
# alert_engine 只消费 metrics 聚合 errors（本进程已不把停牌标的计入 err_count），无需逐标的改动。
WATCH_ENTRIES = {}
        ...
        try:
            ...
            try:
                write_metrics(time.time() - loop_start, 0,
                              err_count + 1, max_bar_ts, len(active))
            except Exception:
                pass
