...
# 信号复算 + 验证
# ----------------------------------------------------------------------------- #
def replay_symbol(sym, name, data, pc, gates='prod', atr_min_pct='auto', mpr='auto'):
    """空 state 复算（与生产单实例单日从零开始等价），返回信号明细 + 日统计。

    [2026-08-11 P0 口径修复] `M.detect_for` 的闸门（mpr / atr_min_pct）**只认函数入参**，
    函数内部不回退读 PER_SYMBOL_CFG。此前本函数调 detect_for 未传任何闸门参数，等价于
    「闸门全关」；而实盘 run() 循环透传 per_symbol_mpr/per_symbol_atr（生产口径为
    mpr_enable='B' + atr_min_pct=0.25）。后果：复算信号**系统性多于**实盘，连续多日
    「live=0 vs recalc>=2」的哨兵告警被误归因为「生产侧落盘断流/首扫抑制」，实际至少
    部分是复算口径自身放宽造成的假象。对账（prod_vs_bt_reconcile）与 G1 执行差也因此失真。

    gates:
      'prod' —— 默认。取 monitor_config.json 生产闸门，与实盘 run() 完全同口径。
      'off'  —— 旧行为（闸门全关），仅供闸门消融/对照实验，**不得用于对账**。
    atr_min_pct / mpr: 'auto' 随 gates 取值；显式传值则覆盖（消融探针 / 网格寻优用）。
    """
    c = data['c']; n = data['n']; df = data['df']
    trade_times = df['trade_time'].values if df is not None else None

    if gates == 'prod':
        _mpr_e, _mpr_p = M.per_symbol_mpr(sym)
        _atr = M.per_symbol_atr(sym)
    elif gates == 'off':
        _mpr_e, _mpr_p, _atr = None, None, None
    else:
        raise ValueError(f"gates 仅支持 'prod'/'off'，收到 {gates!r}")
    if atr_min_pct != 'auto':
        _atr = atr_min_pct
    if mpr != 'auto':
        if isinstance(mpr, (tuple, list)):
            _mpr_e, _mpr_p = mpr
        else:
            _mpr_e = mpr
    ...
    sigs = M.detect_for(sym, name, data, st,
                        mpr_enable=_mpr_e, mpr_periods=_mpr_p, atr_min_pct=_atr)
